...
class AuthorizationServerConfig extends AuthorizationServerConfigurerAdapter {
    ...
    public void configure(ClientDetailsServiceConfigurer clients) throws Exception {
        //这里的数据不可以写死 需要从数据库中取 因为客户端不只有changgou一家
//        clients.inMemory()
//                .withClient("changgou")          //客户端id 就是代理用户的那个传输用户数据的app客户端
//                .secret("changgou")                      //秘钥
...
//                .scopes("app");                         //客户端范围，名称自定义，必填
        ...
        clients.jdbc(dataSource).clients(clientDetails());//表示连接了数据库 也有dao查询数据库，dao就是clientDetails（）
    }
