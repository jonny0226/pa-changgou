...
public class SpecServiceImpl implements SpecService {
    ...
    public List<Spec> findByCategoryId(Integer categoryId) {
        ...
        // select tbs.* from tb_spec tbs ,tb_category tbc where tbs.template_id=tbc.template_id and tbc.id=76

        //1.先根据商品分类ID 获取商品分类对象数据
        Category category = categoryMapper.selectByPrimaryKey(categoryId);
        //2.从对象中获取对应的template_id
        Integer templateId = category.getTemplateId();//42
        //3.根据template_id 获取规格的列表
        Spec condition = new Spec();
        condition.setTemplateId(templateId);// where template_id=42
