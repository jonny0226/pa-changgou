...
public class SkuController {
    ...
    /**
     * 根据状态值查询该状态值对应的所有的数据 如果 1  查询正常的  0 查询不正常的
     * @param status
     * @return
     */
    ...
    Result decCount(@RequestParam(value = "id") Long id,@RequestParam(value = "num") Integer num){
        int count =  skuService.decCount(id,num);//库存减少后的受影响行数
        if (count>=1){

            return  new Result(true,StatusCode.OK,"库存减少成功");
        }else{
            return  new Result(false,StatusCode.ERROR,"库存减少失败");
        }
    }
}
