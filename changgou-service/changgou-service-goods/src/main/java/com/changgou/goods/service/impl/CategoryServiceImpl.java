...
public class CategoryServiceImpl implements CategoryService {
    ...
    public List<Category> findByParentId(Integer pid) {
       ...
       /* condition.setId(1);//where parent_id=? and id=?
        condition.setIsMenu("1");// where parent_id=? and id=? and is_menu="1"*/
        return categoryMapper.select(condition);//根据条件查询   只能是=号的条件  exmaple是任意的条件 》< in not
    }
}
