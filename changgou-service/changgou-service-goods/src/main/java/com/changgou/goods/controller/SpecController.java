...
public class SpecController {
    ...
    /**
     * 根据商品的分类的ID 获取该分类下的所对应的规格的列表
     * @return
     */
    @GetMapping(value = "/category/{id}")
    public Result<List<Spec>> findByCategoryId(@PathVariable(name="id")Integer categoryId){
        List<Spec> specList=  specService.findByCategoryId(categoryId);
        return new Result<List<Spec>>(true, StatusCode.OK,"查询某一个分类下的规格列表成功",specList);
    }
}
